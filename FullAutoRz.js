const Green = {
    ...
    getCallSeconds: () => {
        ...
        if (!timerText.includes(':') || parts.some((part) => Number.isNaN(part))) return null;
    ...
    getShortCallRetry: () => {
        try {
            return JSON.parse(localStorage.getItem(Green.shortCallRetryKey) || "null");
        } catch (e) {
    ...
    markShortCallForRetry: (seconds) => {
        ...
        const previous = Green.getShortCallRetry();
    ...
    bindShortCallRetries: () => {
        ...
        window.addEventListener("storage", (event) => {
            ...
            // confirmation, email, and all later lead transitions.
            Green.setTimeout(() => {
    ...
    autoClickCallOnLoad: () => {
        ...
        // Give the normal lead setup time to bind its click handler first.
        Green.setTimeout(() => {
            Green.clickCallAndConfirm();
        }, 1800, false);
    },
    ...
    initOnConfirm: () => {
        ...
        Green.callIconClick(() => {
            // Retrying the same lead clicks the icon again. Bind the close
            // handler once; later attempts only refresh the active user.
            if (!Green.leadCloseListenerBound) {
                saveAndCloseLeedsPage();
                Green.leadCloseListenerBound = true;
            } else if (typeof saveUserId === "function") {
                saveUserId();
            }
